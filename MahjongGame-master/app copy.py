...
NGROK_ACCESS_TOKEN = "YOUR_ACCESS_TOKEN"
...
def update_hand_counts(room_id: str):
    ...
    for pos in POSITIONS:
        hand = room_data["game_state"]["players_hands"].get(pos, [])
        hand_counts[pos] = len(hand)
    socketio.emit('hand_update', {'hand_counts': hand_counts}, room=room_id)
...
def create_room_route():
    ...
    rooms[room_id] = {
         "players": [username],
         "positions": {username: POSITIONS[0]},
         "sids": {},
         "game_started": False,
         "game_state": {},
         "scores": {}
    }
    return redirect(url_for('game', room_id=room_id) + '?username=' + username)
...
def on_join(data):
    ...
    if username in room_data["players"]:
         room_data["sids"][username] = request.sid
    else:
         room_data["players"].append(username)
         room_data["positions"][username] = POSITIONS[len(room_data["players"]) - 1]
         room_data["sids"][username] = request.sid
    join_room(room)
    emit('player_joined', {'username': username, 'players': room_data["players"]}, room=room)
...
def on_start_game(data):
    ...
    room_data = rooms[room]
    if len(room_data["players"]) < 4:
         emit('error', {'message': 'Need 4 players to start the game.'})
         return
    room_data["game_started"] = True
    if TEST_MODE:
         players_hands, remaining_deck = create_test_deck()
    else:
         deck = create_deck()
         shuffle_deck(deck)
         players_hands, remaining_deck = deal_tiles(deck)
    ...
    room_data["scores"] = {user: 2000 for user in room_data["players"]}
    if TEST_MODE:
          test_instructions = (
               "Test Instructions: " +
               f"North: discard {players_hands['north'][0].name}; " +
               "East: discard one of Tile 5 (bamboo 6) to trigger a Pong meld; " +
               f"South: discard {players_hands['south'][0].name}; " +
               f"West: discard {players_hands['west'][0].name}."
          )
          socketio.emit('chat_message', {'username': 'TEST', 'message': test_instructions}, room=room)
    for username in room_data["players"]:
         position = room_data["positions"][username]
         hand = players_hands.get(position, [])
         hand_data = [{
              'id': tile.id,
              'name': tile.name,
              'suit': tile.suit,
         ...
         if sid:
              socketio.emit('deal_hand', {'hand': hand_data, 'position': position}, room=sid)
    socketio.emit('game_started', {'message': 'Game has started! (Test Mode)', 'current_turn': room_data["game_state"]["current_turn"]}, room=room)
...
def on_draw_tile(data):
    ...
    if not room_data:
         ...
         return
    position = room_data["positions"].get(username)
    if position != room_data["game_state"]["current_turn"]:
         emit('error', {'message': "It's not your turn."})
         return
    deck = room_data["game_state"].get("remaining_deck")
    if not deck or len(deck) == 0:
         emit('error', {'message': 'No more tiles to draw.'})
         return
    drawn_tile = deck.pop(0)
    room_data["game_state"]["players_hands"][position].append(drawn_tile)
    drawn_tile_data = {
         'id': drawn_tile.id,
         'name': drawn_tile.name,
         'suit': drawn_tile.suit,
         'image_path': drawn_tile.image_path
    }
    sid = room_data["sids"].get(username)
    if sid:
         socketio.emit('tile_drawn', {'tile': drawn_tile_data}, room=sid)
    update_hand_counts(room)
...
def on_discard_tile(data):
    ...
    socketio.emit('tile_discarded', discard_data, room=room)
    new_turn = get_next_turn(position)
    room_data["game_state"]["current_turn"] = new_turn
    socketio.emit('turn_update', {'current_turn': new_turn}, room=room)
    update_hand_counts(room)
    win, score = check_win_and_score(room, username)
    if win:
         new_scores = settle_scores(room, username, score)
